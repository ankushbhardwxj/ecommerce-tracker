...
const DeliveryEntity = (props) => {
  ...
  const handleDelivered = async () => {
    await axios({
      method: "PUT",
      url: "http://localhost:8000/api/deliveries/item/updateItemDelivered",
      data: {
        id: props.id,
        delivered: !delivered,
      },
  ...
  const handleTransit = async () => {
    await axios({
      method: "PUT",
      url: "http://localhost:8000/api/deliveries/item/updateTransit",
      data: {
        id: props.id,
        onTransit: !inTransit,
      },
  ...
  const handleLocation = async () => {
    ...
    const success = async (pos) => {
      ...
      await axios({
        method: "PUT",
        url: "http://localhost:8000/api/deliveries/item/updateCoordinates",
        data: {
          id: props.id,
          lat: latitude,
          long: longitude,
    ...
    <Card style={{ width: "500px" }}>
      ...
      <Card.Content extra>
        <Button.Group>
          ...
          {inTransit ? (
            <Button primary onClick={handleTransit} content="In Transit" />
          ) : (
